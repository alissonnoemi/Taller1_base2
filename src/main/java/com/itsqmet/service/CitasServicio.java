...
public class CitasServicio {
    ...
    public void cancelarCita(Long citaId) {
        Citas cita = citasRepositorio.findById(citaId)
                .orElseThrow(() -> new IllegalArgumentException("Cita no encontrada con ID: " + citaId));
        cita.setEstadoCita(Citas.EstadoCita.CANCELADA);
        citasRepositorio.save(cita);
    }

    @Transactional
    public void actualizarFechas(Long citaId, String fechaHoraInicioStr, String fechaHoraFinStr) throws Exception {
        Citas cita = citasRepositorio.findById(citaId)
                .orElseThrow(() -> new IllegalArgumentException("Cita no encontrada con ID: " + citaId));

        DateTimeFormatter formatter = DateTimeFormatter.ISO_DATE_TIME; // Formato ISO usado por FullCalendar
        LocalDateTime nuevaInicio = LocalDateTime.parse(fechaHoraInicioStr, formatter);
        LocalDateTime nuevaFin = LocalDateTime.parse(fechaHoraFinStr, formatter);

        // Validar conflictos
        List<Citas> citasConflictivas = citasRepositorio.findConflictingAppointmentsExcludingSelf(
                cita.getProfesional(), nuevaInicio, nuevaFin, cita.getIdCita());
        if (!citasConflictivas.isEmpty()) {
            throw new IllegalArgumentException("Ya existe una cita para este profesional en el horario seleccionado.");
        }

        cita.setFechaHoraInicio(nuevaInicio);
        cita.setFechaHoraFin(nuevaFin);
        citasRepositorio.save(cita);
    }

    @Transactional
    public List<Citas> obtenerCitasPorCliente(Long clienteId) {
        Cliente cliente = pacienteRepositorio.findById(clienteId)
                .orElseThrow(() -> new IllegalArgumentException("Cliente no encontrado con ID: " + clienteId));
        return citasRepositorio.findByClienteOrderByFechaHoraInicioAsc(cliente);
    }

    // Método unificado para agendar o actualizar (usado en controller)
    @Transactional
    public Citas guardarOCambiarCita(Citas cita) throws Exception {
        if (cita.getIdCita() != null && citasRepositorio.existsById(cita.getIdCita())) {
            return actualizarCita(cita.getIdCita(), cita);
        } else {
            return agendarNuevaCita(cita);
        }
    }
    @Transactional
    public void actualizarFechas(Long citaId, LocalDateTime fechaInicio, LocalDateTime fechaFin) throws Exception {
        // Buscar la cita existente
        Citas cita = citasRepositorio.findById(citaId)
                .orElseThrow(() -> new IllegalArgumentException("Cita no encontrada con ID: " + citaId));

        // Validar superposición de citas para el mismo profesional (excluyendo la propia)
        List<Citas> citasConflictivas = citasRepositorio.findConflictingAppointmentsExcludingSelf(
                cita.getProfesional(), fechaInicio, fechaFin, cita.getIdCita());

        if (!citasConflictivas.isEmpty()) {
            throw new IllegalArgumentException("Ya existe una cita conflictiva para el profesional '" +
                    cita.getProfesional().getNombreCompleto() + "' en el horario seleccionado.");
        }
