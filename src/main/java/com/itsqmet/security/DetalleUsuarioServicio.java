...
public class DetalleUsuarioServicio implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        // 1. Intentar cargar como Cliente
        Optional<Cliente> clienteOptional = clienteRepositorio.findByEmail(email);
        if (clienteOptional.isPresent()) {
            Cliente cliente = clienteOptional.get();
            Collection<? extends GrantedAuthority> authorities = Collections.singletonList(new SimpleGrantedAuthority("ROLE_CLIENTE"));
            return User.builder()
        ...
        // 2. Si no es Cliente, intentar cargar como Negocio
        Optional<Negocio> negocioOptional = negocioRepositorio.findByEmail(email);
        if (negocioOptional.isPresent()) {
            Negocio negocio = negocioOptional.get();
            Collection<? extends GrantedAuthority> authorities = Collections.singletonList(new SimpleGrantedAuthority("ROLE_NEGOCIO"));
            return User.builder()
        ...
        // Si no se encuentra en ninguno de los repositorios, lanzar excepción
        throw new UsernameNotFoundException("No se encontró usuario con el nombre: " + email);
    }
    }